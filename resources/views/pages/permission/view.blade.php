<x-base-layout>
    ...
    @section('pageHeader', 'View Permission')
    @section('innerLinkBreadCumb')
        {!!innerBreadCrum($breadCumb)!!}
    @endsection
    @section('headerToolBox')
        <div>
            <a href="{{route('permission.index') }}" class=" mx-12 btn btn-sm btn-primary float-end text-nowrap"><i
...
<!--begin::Card-->
    <div class="card">
        ...
        <div class="card-header flex-wrap pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label"> {{ __('Permission Detail') }}</h3>
            </div>
        </div>
        <!-- end::Card Header -->
        <!--begin::Card body-->
    ...
    <div class="card mt-10">
        ...
        <div class="card-header flex-wrap pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label"> {{ __('Associated Roles') }}</h3>
            </div>
        </div>
        <!-- end::Card Header -->
        <!--begin::Card body-->
        <div class="card-body pt-6">
            <table class="table align-middle table-row-dashed fs-6 gy-5 kt_table" id="kt_customers_table">
                ...
                @foreach($associated as $l)
                    <tr>
                        <!--begin::S#-->
                        <td>{{ $loop->iteration }}</td>
                        <!--end::S#-->
                        <!--begin::module name-->
                        <td>{{ ucwords($l->role) }}</td>
                        <!--end::module name-->
                    </tr>
                @endforeach
                </tbody>
