...
<div id="kt_header" style="" class="header {{ theme()->printHtmlClasses('header', false) }} align-items-stretch" {{ theme()->printHtmlAttributes('header') }}>
    <!--begin::Brand-->
    <div class="header-brand" style="background-color: #1E1E2D;">
        <!--begin::Logo-->
        <a href="{{ theme()->getPageUrl('') }}">
            <img alt="Logo" src="{{ asset(theme()->getMediaUrlPath() . 'logos/light_logo.png') }}" class="h-60px h-lg-60px"/>
        </a>
        <!--end::Logo-->

        @if (theme()->getOption('layout', 'aside/minimize') === true)
            <!--begin::Aside minimize-->
            <div id="kt_aside_toggle" class="btn btn-icon w-auto px-0 btn-color-danger btn-active-color-primary aside-minimize"
                data-kt-toggle="true"
