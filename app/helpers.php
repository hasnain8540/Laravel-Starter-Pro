...
if (!function_exists('bootstrap')) {
    ...
    function bootstrap()
    {
        ...
        if (!class_exists($bootstrap)) {
            abort(404, 'System not been set or '.$bootstrap.' file is not found.');
        }
...
if(!function_exists('equliseLength')) {
    /**
     * Equlize Sting Length with Prefix zeros "0"
     *
     * @return string
     */
    function equliseLength($p_no,$length)
    {
        for($i=strlen($p_no);$i<$length;$i++)
            $p_no="0".$p_no;
        return $p_no;
    }
}
