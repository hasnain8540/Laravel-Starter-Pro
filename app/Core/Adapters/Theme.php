...
class Theme extends \App\Core\Theme
{
    ...
    public static function getOption($scope, $path = false, $default = null)
    {
        ...
        // Map the config path
        if (array_key_exists($scope, config('local'.'.general', []))) {
            $scope = 'general.'.$scope;
        }
        ...
        // Local config
        $localConfig = config('local'.'.'.$scope.$deepPath, $default);

        // check if it is a callback
        if (is_callable($localConfig) && !is_string($localConfig)) {
    ...
    public function initConfig()
    {
        ...
        $mergedConfig = $mainConfig->replaceRecursive($localConfig);
        config(['local' => $mergedConfig->all()]);

        self::$config = $mergedConfig->all();

        // Get config by url path
        $configPath = 'local'.'.pages.'.str_replace('/', '.', Theme::getPagePath());
        $pageConfig = collect(config($configPath));

        // Merge group config with child config
        $pageGroupOptions = Theme::getPageGroupOptions(config('local'.'.pages'), Theme::getPagePath());
        if ($pageGroupOptions) {
            $overridenConfig = $pageConfig->replaceRecursive($pageGroupOptions);
            config([$configPath => $overridenConfig->all()]);
        }

        $generalConfig = collect(config('local'.'.general'));
        // Merge general config with page level config
        config(['local'.'.general' => $generalConfig->replaceRecursive(config($configPath))->all()]);
